...
print("Wyniki dla Regresji liniowej z StandardScaler (porównanie pełnego i skróconego zestawu danych):")
print(results_df)

# Zapisywanie wyników w formacie LaTeX
latex_table = results_df.to_latex(index=False, float_format="%.2f", 
                                  caption="Wyniki modelu Regresji liniowej z StandardScaler dla pełnego i skróconego zestawu danych.",
                                  label="tab:wyniki_lr_scaled_comparison")
...
plt.plot(test_niespokojny_full["timestamp"], y_pred_niespokojny_full, label='Prognozy Regresji liniowej (pełny zestaw)', color='red', linestyle='--')
plt.title('Prognozy cen energii (Regresja liniowa z StandardScaler, okres niespokojny 2023, pełny zestaw)')
plt.xlabel('Data')
plt.ylabel('Cena (PLN/MWh)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("../../plots/prognozy_lr_scaled_niespokojny_full.png", dpi=300)
plt.close()
...
plt.plot(test_niespokojny_short["timestamp"], y_pred_niespokojny_short, label='Prognozy Regresji liniowej (skrócony zestaw)', color='red', linestyle='--')
plt.title('Prognozy cen energii (Regresja liniowa z StandardScaler, okres niespokojny 2023, skrócony zestaw)')
plt.xlabel('Data')
plt.ylabel('Cena (PLN/MWh)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("../../plots/prognozy_lr_scaled_niespokojny_short.png", dpi=300)
plt.close()

print("Wykresy zapisane w ../../plots/prognozy_lr_scaled_niespokojny_full.png i ../../plots/prognozy_lr_scaled_niespokojny_short.png")