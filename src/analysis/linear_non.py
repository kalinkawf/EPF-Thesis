...
train_spokojny_full = df_full[(df_full["timestamp"].dt.year >= 2020) & (df_full["timestamp"].dt.year <= 2022)]
test_spokojny_full = df_full[df_full["timestamp"].dt.year == 2023]
...
# 3. Standaryzacja danych
train_spokojny_short = df_short[(df_short["timestamp"].dt.year >= 2020) & (df_short["timestamp"].dt.year <= 2022)]
test_spokojny_short = df_short[df_short["timestamp"].dt.year == 2023]
...
print("\nRegresja liniowa (pełny zbiór danych) - Okres niestabilny (2023):")
...
predictions_df_full_linear.to_csv("../../results/predictions_full_linear_not_stable.csv", index=False)
...
print("\nRegresja liniowa (skrócony zbiór danych) - Okres niestabilny (2023):")
...
predictions_df_short_linear.to_csv("../../results/predictions_short_linear_not_stable.csv", index=False)
...
print("\nRegresja Ridge (pełny zbiór danych) - Okres niestabilny (2023):")
...
predictions_df_full_ridge.to_csv("../../results/predictions_full_ridge_not_stable.csv", index=False)
...
print("\nRegresja Ridge (skrócony zbiór danych) - Okres niestabilny (2023):")
...
predictions_df_short_ridge.to_csv("../../results/predictions_short_ridge_not_stable.csv", index=False)
...
plt.figure(figsize=(12, 4))
plt.plot(test_spokojny_full.loc[sep_oct_mask, "timestamp"], y_test_full_original[sep_oct_mask], label="Rzeczywiste", color="blue", alpha=0.7)
plt.plot(test_spokojny_full.loc[sep_oct_mask, "timestamp"], ridge_predictions_full_original[sep_oct_mask], label="Regresja Ridge", color="red", alpha=0.7, linestyle="--")
plt.title("Okres niestabilny (Wrzesień - Październik 2023)", fontsize=14)
plt.xlabel("Czas", fontsize=12)
plt.ylabel("Cena energii [PLN/MWh]", fontsize=12)
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig("../../plots/predicts/ridge_predictions_full_sep_oct_2023.png", dpi=300)
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("../../plots/predicts/errors_vs_predictions_Ridge_full_not_stable_period.png", dpi=300)
plt.close()
