...
# Filtrowanie danych: trenowanie 2020-2022, testowanie 2023
train_full = df_full['2020-01-01':'2022-12-31'].reset_index()
test_full = df_full['2023-01-01':'2023-12-31'].reset_index()
...
train_short = df_short['2020-01-01':'2022-12-31'].reset_index()
test_short = df_short['2023-01-01':'2023-12-31'].reset_index()
...
polish_holidays = pd.DataFrame({
    ...
    'ds': pd.to_datetime([
        # Stałe święta w Polsce (2020-2023)
        '2020-01-01', '2021-01-01', '2022-01-01', '2023-01-01',  # Nowy Rok
        '2020-01-06', '2021-01-06', '2022-01-06', '2023-01-06',  # Trzech Króli
        '2020-05-01', '2021-05-01', '2022-05-01', '2023-05-01',  # Święto Pracy
        '2020-05-03', '2021-05-03', '2022-05-03', '2023-05-03',  # Konstytucja 3 Maja
        '2020-08-15', '2021-08-15', '2022-08-15', '2023-08-15',  # Wniebowzięcie NMP
        '2020-11-01', '2021-11-01', '2022-11-01', '2023-11-01',  # Wszystkich Świętych
        '2020-11-11', '2021-11-11', '2022-11-11', '2023-11-11',  # Dzień Niepodległości
        '2020-12-25', '2021-12-25', '2022-12-25', '2023-12-25',  # Boże Narodzenie (pierwszy dzień)
        '2020-12-26', '2021-12-26', '2022-12-26', '2023-12-26',  # Boże Narodzenie (drugi dzień)
        # Wielkanoc i Zielone Świątki (zmienne daty w zależności od roku)
        '2020-04-12', '2021-04-04', '2022-04-17', '2023-04-09',  # Wielkanoc (niedziela)
        '2020-04-13', '2021-04-05', '2022-04-18', '2023-04-10',  # Poniedziałek Wielkanocny
        '2020-06-01', '2021-05-23', '2022-06-05', '2023-05-28',  # Zielone Świątki (Zesłanie Ducha Świętego)
        '2020-06-11', '2021-06-03', '2022-06-16', '2023-06-08',  # Boże Ciało
    ]),
    'lower_window': 0,  # Dzień święta
    'upper_window': 1,  # Dzień po święcie
})

# Definicja kombinacji parametrów Prophet (z poprzedniego kroku, bez multiplicative)
param_combinations = [
    {'changepoint_prior_scale': 0.001, 'seasonality_prior_scale': 50.0, 'holidays_prior_scale': 0.1, 'seasonality_mode': 'additive'},
    # {'changepoint_prior_scale': 0.001, 'seasonality_prior_scale': 1.0, 'holidays_prior_scale': 10.0, 'seasonality_mode': 'additive'},
    # {'changepoint_prior_scale': 0.001, 'seasonality_prior_scale': 5.0, 'holidays_prior_scale': 1.0, 'seasonality_mode': 'additive'},
    # {'changepoint_prior_scale': 0.001, 'seasonality_prior_scale': 1.0, 'holidays_prior_scale': 0.1, 'seasonality_mode': 'additive'},
    # {'changepoint_prior_scale': 0.001, 'seasonality_prior_scale': 10.0, 'holidays_prior_scale': 1.0, 'seasonality_mode': 'additive'},
...
def evaluate_prophet(train_data, test_data, params, combination_idx, dataset_type, regressors):
    ...
    # plt.savefig(f'../../plots/predicts/residuals_histogram_Prophet_unstable_{dataset_type.lower()}_comb_{combination_idx}.png', dpi=300)
    # plt.close()
    ...
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig(f"../../plots/predicts/errors_over_time_Prophet_non_stable.png", dpi=300)
    # plt.close()
    
    # # Wykres predykcji vs. wartości rzeczywistych dla wrzesień - sierpień 2023
    # test_data_sep_aug = test_data[(test_data['ds'] >= '2023-09-01') & (test_data['ds'] < '2023-11-01')]
    # y_true_sep_aug = test_data_sep_aug['y'].values
    # y_pred_sep_aug = forecast.loc[forecast['ds'].isin(test_data_sep_aug['ds']), 'yhat'].values

    # plt.figure(figsize=(12, 4))
    # plt.plot(test_data_sep_aug['ds'], y_true_sep_aug, label="Rzeczywiste", color="blue", alpha=0.7)
    # plt.plot(test_data_sep_aug['ds'], y_pred_sep_aug, label="Prophet", color="red", alpha=0.7, linestyle='--')
    ...
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig(f'../../plots/predicts/Prophet_predictions_unstable_sep_{dataset_type.lower()}_comb_{combination_idx}.png', dpi=300)
    # plt.close()

    # Wykres top 50 błędów
    ...
    # plt.title(f"Top 50 błędów: Predykcje vs Rzeczywiste (Prophet, {dataset_type} zbiór)", fontsize=12)
    # plt.xlabel("Predykcje [PLN/MWh]", fontsize=10)
    # plt.ylabel("Rzeczywiste wartości [PLN/MWh]", fontsize=10)
    # plt.grid(True)
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig(f"../../plots/predicts/top_50_errors_Prophet_unstable_{dataset_type.lower()}_period_comb_{combination_idx}.png", dpi=300)
    # plt.close()
...
results_full_df.to_csv('../../results/prophet_results_full_unstable.csv', index=False)
results_short_df.to_csv('../../results/prophet_results_short_unstable.csv', index=False)
